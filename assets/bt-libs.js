var BT = {
  ...
  getVariantPriceHtml: function(pg, variant) {
    ...
    if(variant.compare_at_price > variant.price) {
      priceHtml += '<s class="product-price__price">' + this.getPriceHtml(variant.compare_at_price) + '</s>';
      priceHtml += '<span class="product-price__price product-price__sale">' + this.getPriceHtml(variant.price) + '</span>';
    } else {
        ...
        ins.loadCssJsLib('product-global', function() {
          ins.loadSnippetAjax('popup-quickview', function() {
            ins.callAjax(url, 'GET', {view: 'quick_view'}, null, function(html) {
              ...
              qvContentEle.html(ins.stripScripts(html));
              ...
              ins.init3dModel($(qvPopupSelector), 'quickview-' + jsonScript.attr('data-handle'));
            });
          });
        });
      });
  ...
  initSelectOption: function() {
    ...
    $(document).on('click', this.selectors.selectOptionBtn, function(e) {
      ...
      $(this).addClass(workingClass);
      var url = $(this).attr('data-href'), btn = $(this);
      ins.startLoadingBtn(btn);
      ins.loadSingleGeneralCss(function() {
  ...
  loadCompare: function(cb) { 
    ...
    this.loadSingleGeneralCss(function() {
      ins.callAjax(theme.searchUrl, 'GET', {
        view: 'compare',
  ...
  initCompareEvent: function() {
    ...
    $(document).on('click', this.selectors.compare.btn, function(e) {
      ...
      if(ins.isProductExistInCookie(id, ins.options.compare.cookieName) && $(ins.selectors.compare.popup).find('table').length > 0) {
        ins.showPopup(ins.selectors.compare.popup);
      } else {
        ins.addProductToCookie(id, ins.options.compare.cookieName);
        ins.startLoadingBtn(btn);
    ...
    $(document).on('click', this.selectors.compare.removeBtn, function(e) {
      ...
      ins.removeProductFromCookie(id, ins.options.compare.cookieName);
      ins.loadCompare();
    });
  ...
  afterLoadDynamicProductsAjaxInTab: function(parent, html) {
    ...
    ins.initSlider(parent.parents('.tab-pane'), false);
  ...
  popularAddedWishlistItems: function(wrap){
    ...
    var btnSelector = '', btnTooltipSelector = '', btnWithTextSelector = '', textSelector = '';
    var items = this.getCookieItemsValue(true, cookieName);
    if(items.length > 0) {
     $.each(items, function( index, value ) {
        ...
        btnTooltipSelector += selector.btn + '[data-id="' + value + '"]' + '[data-toggle="tooltip"]';
    ...
    wrap.find(btnTooltipSelector).tooltip('fixTitle');
  ...
  appendWishlistEmtptyStr: function(popupContent) {
    ...
    popupContent.html('<div class="alert alert-warning"><span>' + theme.strings.wishlistEmpty +'</span></div>');
  },
  ...
  loadProductColumnsAjax: function($container) {
    ...
    if($container.find('.waiting-scroll-data').length > 0) {
      this.loadCssJsLib('waypoint', function() {
        $container.find('.waiting-scroll-data').each(function() {
          ...
          var wp = trigger.waypoint(function(direction) {
            ...
            trigger.remove();
            ins.callAjax(theme.rootUrl, 'GET', {view: 'product_columns_ajax', q: trigger.attr('data-ajax-params')}, null, function(html) {
              parent.html($('.products', html).html());
              ins.convertCurrencySilence(parent.find('span.money'));
              parent.removeClass('waiting-data');
              if(parent.hasClass('external-arrows')) {
                parent.siblings('.pcol__title').find('.slick-arrow-title').html('');
              }
              ins.initSlider(parent.parent(), false);
